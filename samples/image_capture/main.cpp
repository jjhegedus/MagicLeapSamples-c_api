...
public:
  ...
  void OnUpdate(float) override {
    ...
    if (camera_handle_ == ML_INVALID_HANDLE) {
      UNWRAP_MLRESULT(MLCameraSetOutputFormat(FLAGS_jpeg ? MLCameraOutputFormat_JPEG : MLCameraOutputFormat_YUV_420_888)); // only effects image capture
      UNWRAP_MLRESULT(MLCameraPrepareCapture((FLAGS_video ? MLCameraCaptureType_VideoRaw : MLCameraCaptureType_ImageRaw), &camera_handle_));
      if (FLAGS_video) {
      ...
      if (capture_status & MLCameraCaptureStatusFlag_Completed) {
        ML_LOG(Info, "Frame #: %llu Timestamp %llu", TO_LLU(extras->frame_number), TO_LLU(extras->vcam_timestamp_us));
        MLCameraOutput *data = nullptr;
        MLResult ret = MLCameraGetImageStream(&data);
        if (MLCameraGetImageStream(&data) == MLResult_Ok) {
          save_path_ = std::string(GetLifecycleInfo().writable_dir_path) + "img_" + CurrentDateTime() + "_" + std::to_string(extras->frame_number);
          if (FLAGS_video) {
            save_path_ += ".yuv";
          } else {
            if (FLAGS_jpeg) {
...
private:
  bool SetUpCamera() {
    ...
    if (MLPrivilegesResult_Granted != result) {
      ML_LOG(Error, "ImageCapture: Error requesting MLPrivilegeID_AudioCaptureMic privilege, error: %s", MLPrivilegesGetResultString(result));
      return false;
    }
