...
int main() {
  ...
  while (AppStatus::Stopped != status) {
    ...
    for (int camera = 0; camera < 2; ++camera) {
      ...
      glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, frame_info.virtual_camera_info_array.color_id, 0, camera);
      glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, frame_info.virtual_camera_info_array.depth_id, 0, camera);

      const MLRectf &viewport = frame_info.virtual_camera_info_array.viewport;
      ...
      UNWRAP_MLRESULT(
          MLGraphicsSignalSyncObjectGL(graphics_client, frame_info.virtual_camera_info_array.virtual_cameras[camera].sync_object));
    }
