...
Application::Application(int argc, char **argv) {
  ...
  fps_delta_time_ = ApplicationClock::now();
...
void Application::InitializeLifecycle() {
  ...
  lifecycle_callbacks.on_device_active = [](void *) {
    ML_LOG(Info, "On active called.");
  };
  lifecycle_callbacks.on_device_reality = [](void *) {
    ML_LOG(Info, "On reality called.");
  };
  lifecycle_callbacks.on_device_standby = [](void *) {
...
void Application::Update() {
  ...
  num_frames_++;
  auto d = std::chrono::duration_cast<std::chrono::seconds>(update_time - fps_delta_time_);
  if (d.count() >= 1.0) {
    ML_LOG(Verbose, "%f ms/frame (fps: %u)", 1000.0/double(num_frames_), num_frames_);
    num_frames_ = 0;
    fps_delta_time_ += d;
  }
...
void Application::Render() {
  ...
  if (MLResult_Pending == out_result || MLResult_Timeout == out_result) {
    ...
    UpdateMLCamera(frame_info.virtual_camera_info_array);
    renderer_.Render();
...
void Application::UpdateMLCamera(const MLGraphicsVirtualCameraInfoArray &camera_info) {
  using namespace ml::app_framework;

  for (uint32_t camera_index = 0; camera_index < camera_info.num_virtual_cameras; ++camera_index) {
    std::shared_ptr<CameraComponent> cam = camera_nodes_[camera_index]->GetComponent<CameraComponent>();

    const MLRectf &viewport = camera_info.viewport;
    cam->SetRenderTarget(ml_render_target_cache_[std::make_pair(camera_info.color_id, camera_index)]);
    ...
    auto &current_camera = camera_info.virtual_cameras[camera_index];
